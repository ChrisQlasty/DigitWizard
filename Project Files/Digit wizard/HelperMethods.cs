...
namespace FF_TouchlessControllerViewer.cs
{
    ...
    public static class HelperMethods
    {
        ...
        public static bool PointOnCanvas(Point point, Canvas canvas, Grid containinGrid)
        {
            var p = canvas.TranslatePoint(new Point(), containinGrid);

            return (p.X <= point.X * containinGrid.ActualWidth) &&
                   ((p.X + canvas.ActualWidth) >= point.X * containinGrid.ActualWidth) &&
                   ((p.Y + canvas.ActualHeight) >= point.Y * containinGrid.ActualHeight) &&
                   (p.Y <= point.Y * containinGrid.ActualHeight);
        }
    }
